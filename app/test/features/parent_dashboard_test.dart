...
void main() {
  ...
  Future<(int, int)> seed() async {
    ...
    await db.feedsDao
        .savePlayerPosition(profileId: ada, workId: w1, tMs: 5 * 60000);
    final course = await db.studyDao
        .importCourse(profileId: ada, raw: ladderCourse(), nowMs: 1000);
  ...
  testWidgets('shows a calm per-profile card of what each reader has built',
    ...
    expect(find.text('Current course: A Ladder of Two — 1 of 2 mastered'),
        findsOneWidget);
  ...
  testWidgets('rename flows through to the database and the card',
    ...
    await tester.enterText(
        find.byKey(const Key('rename-field')), 'Ada Lovelace');
