...
/// Suggested [Grade] for a free-recall (qa / procedure) attempt.
///
/// With at least one non-empty `acceptable` anchor, delegate to
/// [keywordCoverage] + [suggestGrade] as before. With **no** usable anchors
/// there is no coverage signal at all, so don't bias the learner down by
/// mapping "0.0 coverage" to Again: a non-empty attempt suggests [Grade.good]
/// (the neutral nudge — the honest self-rating is still the learner's), and
/// only an empty/whitespace attempt suggests [Grade.again].
Grade suggestFreeRecallGrade(List<String> acceptable, String response) {
  final hasAnchor = acceptable.any((a) => normalizeAnswer(a).isNotEmpty);
  if (!hasAnchor) {
    return response.trim().isEmpty ? Grade.again : Grade.good;
  }
  return suggestGrade(keywordCoverage(acceptable, response));
}
