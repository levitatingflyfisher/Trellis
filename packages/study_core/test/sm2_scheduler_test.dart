...
void main() {
  ...
  group('scheduleSm2 — a successful recall never shortens the interval', () {
    test('repeated "hard" keeps the interval growing', () {
      // "Hard" applies a 0.6 multiplier on top of interval x ease, and each
      // hard rating also drops ease by 0.14 toward the 1.3 floor. Once ease
      // falls below ~1.67 the product is < 1, so a card the learner KEPT
      // GETTING RIGHT started coming back sooner and sooner — measured
      // 1, 4, 5, 6, 7, 7, 7, 6, 5, 4, 3, 2, 2, 2... forever. Classic SM-2
      // lets quality shape how fast the interval grows, never whether it
      // grows: shrinking is reserved for a lapse.
      var card = CardState.initial('h', const SrsDefaults(), 0);
      final intervals = <int>[];
      for (var i = 0; i < 20; i++) {
        card = scheduleSm2(card, Grade.hard, card.dueEpochDay);
      ...
      for (var i = 1; i < intervals.length; i++) {
        expect(
          ...
          reason: 'review ${i + 1} scheduled sooner than review $i '
              '(sequence: $intervals)',
        );
      }
    });

    test('every grade above a lapse extends a graduated card', () {
      for (final grade in [Grade.hard, Grade.good, Grade.easy]) {
        // Low ease is where the multipliers turn destructive.
        ...
        final next = scheduleSm2(card, grade, 0);
        expect(next.intervalDays, greaterThan(card.intervalDays),
            reason: '"${grade.name}" shortened the interval of a card the '
                'learner recalled correctly');
      }
    });

    test('a lapse still resets — shrinking belongs to failure only', () {
      const card = CardState(
        itemId: 'g',
        ease: 2.0,
        intervalDays: 30,
      ...
      final next = scheduleSm2(card, Grade.again, 0);
      expect(next.intervalDays, 0);
      expect(next.dueEpochDay, 0);
