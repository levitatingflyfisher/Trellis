...
import 'package:study_core/src/grading.dart';
...
void main() {
  ...
  group('suggestFreeRecallGrade', () {
    test('no anchors + a real answer -> good (not again)', () {
      // Regression: with no anchors keywordCoverage is 0.0, so the raw
      // suggestGrade path pre-highlighted 'Again' even for a perfect answer.
      expect(
        suggestFreeRecallGrade(const [], 'a perfectly recalled answer'),
        Grade.good,
      );
    });

    test('no anchors + empty/whitespace-only response -> again', () {
      expect(suggestFreeRecallGrade(const [], ''), Grade.again);
      expect(suggestFreeRecallGrade(const [], '   \n\t'), Grade.again);
    });

    test('anchors that normalize to empty count as no anchors', () {
      expect(
        suggestFreeRecallGrade(const ['', '   '], 'wrote something'),
        Grade.good,
      );
      expect(suggestFreeRecallGrade(const ['', '   '], ' '), Grade.again);
    });

    test('with real anchors it still delegates to coverage', () {
      expect(suggestFreeRecallGrade(const ['x'], 'x marks the spot'),
          Grade.easy); // 1/1 coverage
      expect(suggestFreeRecallGrade(const ['regime', 'markov'], 'no idea'),
          Grade.again); // 0 coverage
      expect(
          suggestFreeRecallGrade(
              const ['regime', 'markov', 'filters', 'mixing'], 'just markov'),
          Grade.hard); // 1/4 coverage
    });
  });
